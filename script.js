const show_meme = document.querySelector('.meme');
const show_joke = document.querySelector('.joke');
const show_quote = document.querySelector('.quote')
const show_riddle = document.querySelector('.riddle_question');
const show_answer = document.querySelector('.riddle_answer');
...
function clearAll() {
  show_meme.innerHTML=null;
  show_joke.innerHTML=null;
  show_quote.innerHTML=null;
}
...
function showJoke() {
  ...
  show_joke.appendChild(text);
}

function showQuote() {
  clearAll();
  const randomQuote = getRandomData("quotes");
  text.textContent = 'Quote: '+randomQuote.quote+'\nAuthor: '+randomQuote.author
  show_quote.appendChild(text);
}

function showRiddle() {
  randomRiddle = getRandomData("riddles");
  text.textContent = 'Question: '+randomRiddle.question
  show_riddle.appendChild(text);
  answer = text.textContent + '\nAnswer: '+ randomRiddle.answer;
  
}

function revealAnswers() {
  child = show_riddle.children.item(0);
  if (show_riddle.innerHTML === '') {
    alert('There is no riddle to show the answer!');
  } else if (child.textContent === answer) {
